...
import mergeRefs from '../react-dnd/merge-refs';
...
type DraggableState = 'idle' | 'dragging' | 'file-over';
...
export const Card = memo(function Card({ item, index }: { item: Item; index: number }) {
  ...
  const [{ canDropFile, isFileOver }, fileDropRef] = useDrop(
    () => ({
  ...
  return (
    <Draggable draggableId={item.itemId} index={index}>
      {(provided: DraggableProvided, snapshot: DraggableStateSnapshot) => {
        const state: DraggableState = snapshot.isDragging
          ? 'dragging'
          : isFileOver
          ? 'file-over'
          : 'idle';
        return (
          <div
            css={cardStyles}
            ref={mergeRefs([provided.innerRef, fileDropRef])}
